...
function inArray(k,a){
  for(var i=0;i<a.length;i++) if(a[i]===k) return true;
  return false;
}
...
function HTML_month(m,y) {
  ...
  for(var _d=mfdn; _d<mtot+mfdn; _d++){
    ...
    // Create new week
    if(_d7===0) MONTH += "</ul><ul class='calendar_week'>";
...
$calMs.on("mousedown.calendar", ".calendar_day_enabled:not(.calendar_day_range_disabled)", function(){
  if($lastSelected) $lastSelected.not(this).removeClass("calendar_day_active");
  $lastSelected = $(this);
...
$calMs.on("mouseenter.calendar", ".calendar_day_enabled:not(.calendar_day_range_disabled)", function(){
  ...
  if($rangeHoverDays) $rangeHoverDays.removeClass("calendar_day_range_hover calendar_day_range_first calendar_day_range_last");
});
